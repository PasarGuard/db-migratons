...
class UniversalMigrator:
    ...
    def __init__(self, source: str, target_type: str, target_url: str, source_type: str = None):
        self.source = source
        self.source_type = source_type  # 'postgres', 'mysql', 'sqlite', or None for auto-detect
    ...
    def detect_source_type(self) -> bool:
        ...
        if any(self.source.startswith(prefix) for prefix in ['postgresql://', 'mysql://', 'sqlite://']):
            self.is_source_live = True
    ...
    async def connect_source(self):
        ...
        if self.is_source_live:
            if self.source_type == 'sqlite':
                ...
                self.source_engine = create_engine(source_url, pool_pre_ping=True, echo=False)
            else:
                self.source_engine = create_engine(self.source, pool_pre_ping=True, echo=False)
            print(f"✓ Connected to source {self.source_type.upper()}")

    async def connect_target(self):
        """Connect to target database"""
        if self.is_target_async:
            self.target_engine = create_async_engine(self.target_url, pool_pre_ping=True, echo=False)
            self.session_maker = async_sessionmaker(self.target_engine, class_=AsyncSession, expire_on_commit=False)
        else:
            self.target_engine = create_engine(self.target_url, pool_pre_ping=True, echo=False)
        ...
        if self.is_target_async:
            async with self.session_maker() as session:
                result = await session.execute(
                    text("SELECT column_name, udt_name, is_nullable, column_default FROM information_schema.columns WHERE table_name = :t ORDER BY ordinal_position"),
                    {"t": table}
                )
            ...
            with self.target_engine.connect() as conn:
                if self.target_type == 'mysql':
                    result = conn.execute(
                        text("SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE, COLUMN_DEFAULT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = :t ORDER BY ORDINAL_POSITION"),
                        {"t": table}
                    )
                    return {row[0]: {'type': row[1].lower(), 'nullable': row[2] == 'YES', 'default': row[3]} for row in result.fetchall()}
                else:  # sqlite
                    result = conn.execute(text(f"PRAGMA table_info({table})"))
                    return {row[1]: {'type': row[2].lower(), 'nullable': row[3] == 0, 'default': row[4]} for row in result.fetchall()}
    ...
    def _get_default_value(self, col_info: dict, table: str, column: str):
        ...
        if any(t in col_type for t in ['float', 'real', 'double', 'numeric', 'decimal']):
            return 0.0
    ...
    async def clear_data(self):
        ...
        print("\nClearing tables...")
        tables = [
            "admin_usage_logs", "user_usage_logs", "user_subscription_updates",
            "notification_reminders", "node_user_usages", "node_usages", "node_stats",
            "next_plans", "users_groups_association", "inbounds_groups_association",
            "template_group_association", "users", "user_templates", "hosts",
            "inbounds", "groups", "nodes", "core_configs", "admins", "system", "jwt", "settings"
        ]
    ...
    async def import_data(self):
        ...
        order = [
            "jwt", "system", "settings", "admins", "core_configs", "nodes",
            "inbounds", "groups", "inbounds_groups_association", "hosts",
            "user_templates", "template_group_association", "users",
            "users_groups_association", "next_plans", "admin_usage_logs",
            "user_usage_logs", "notification_reminders", "user_subscription_updates",
            "node_user_usages", "node_usages", "node_stats"
        ]
                ...
                async with self.session_maker() as session:
                    ...
                    for row_vals in rows:
                        ...
                        for c, v in zip(cols, row_vals):
                            col_info = cols_dict.get(c, {'type': 'text', 'nullable': True, 'default': None})
    ...
    def _convert_create_table_to_sqlite(self, table: str, definition: str) -> str:
        ...
        definition = re.sub(r'\bBIGINT\(\d+\)', 'INTEGER', definition, flags=re.IGNORECASE)
        definition = re.sub(r'\bINT\(\d+\)', 'INTEGER', definition, flags=re.IGNORECASE)
        definition = re.sub(r'\bSMALLINT\(\d+\)', 'INTEGER', definition, flags=re.IGNORECASE)
        definition = re.sub(r'\bTINYINT\(\d+\)', 'INTEGER', definition, flags=re.IGNORECASE)
        definition = re.sub(r'\bVARCHAR\(\d+\)', 'TEXT', definition, flags=re.IGNORECASE)
        ...
        definition = re.sub(r'\bDECIMAL\(\d+,\d+\)', 'REAL', definition, flags=re.IGNORECASE)
        ...
        definition = re.sub(r'\bAUTO_INCREMENT\b', 'AUTOINCREMENT', definition, flags=re.IGNORECASE)
        ...
        definition = re.sub(r'\bCHARACTER SET\s+\w+', '', definition, flags=re.IGNORECASE)
        definition = re.sub(r'\bON UPDATE CURRENT_TIMESTAMP\b', '', definition, flags=re.IGNORECASE)
        definition = re.sub(r'\bCOMMENT\s+\'[^\']*\'', '', definition, flags=re.IGNORECASE)

        # Remove KEY definitions (indexes)
        definition = re.sub(r',\s*PRIMARY KEY\s*\([^)]+\)', '', definition, flags=re.IGNORECASE)
        definition = re.sub(r',\s*KEY\s+\w+\s*\([^)]+\)', '', definition, flags=re.IGNORECASE)
        definition = re.sub(r',\s*UNIQUE KEY\s+\w+\s*\([^)]+\)', '', definition, flags=re.IGNORECASE)
        definition = re.sub(r',\s*INDEX\s+\w+\s*\([^)]+\)', '', definition, flags=re.IGNORECASE)
        definition = re.sub(r',\s*FULLTEXT\s+KEY\s+\w+\s*\([^)]+\)', '', definition, flags=re.IGNORECASE)

        # Remove CONSTRAINT FOREIGN KEY definitions
        definition = re.sub(r',\s*CONSTRAINT\s+\w+\s+FOREIGN KEY\s*\([^)]+\)\s*REFERENCES\s+\w+\s*\([^)]+\)(?:\s+ON\s+DELETE\s+\w+)?(?:\s+ON\s+UPDATE\s+\w+)?', '', definition, flags=re.IGNORECASE)
    ...
    async def create_schema(self):
        ...
        if self.target_type == 'sqlite' and self.create_statements:
            ...
            with self.target_engine.begin() as conn:
                for table in [
                    "jwt", "system", "settings", "admins", "core_configs", "nodes",
                    "inbounds", "groups", "hosts", "user_templates", "users",
                    "inbounds_groups_association", "template_group_association",
                    "users_groups_association", "next_plans", "admin_usage_logs",
                    "user_usage_logs", "notification_reminders", "user_subscription_updates",
                    "node_user_usages", "node_usages", "node_stats"
                ]:
                    if table in self.create_statements:
                        try:
                            create_sql = self._convert_create_table_to_sqlite(table, self.create_statements[table])
    ...
    async def run(self):
        """Run migration"""
        print("="*50)
        print(f"Database Migration: {self.source_type.upper() if self.source_type else 'SQL'} → {self.target_type.upper()}")
        print("="*50)
...
async def main():
    ...
    if not os.path.exists(source) and not source.startswith(('postgresql://', 'mysql://', 'sqlite://')):
